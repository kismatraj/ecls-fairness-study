...
\begin{abstract}
School districts increasingly use predictive algorithms to identify students at risk of academic failure, yet little is known about whether these tools work equitably across demographic groups over time. We audited seven machine learning algorithms predicting 5th-grade risk from kindergarten-through-2nd-grade data in the nationally representative ECLS-K:2011 ($N = 9{,}104$; sensitivity analyses on $N = 18{,}151$). All seven---from simple regression to state-of-the-art gradient boosting---produced nearly identical results (AUC = 0.837--0.848), indicating that inequity lies in the data, not the algorithm. The disparities were substantial: Hispanic students were flagged at 2.4 times the rate of White students, Black students experienced 3.7 times higher calibration error, and these gaps persisted even with additional years of data. Counterfactual decomposition traced these disparities to differences in children's baseline characteristics---particularly early cognitive scores that already reflect socioeconomic stratification at school entry. The gap attributable to baseline differences exceeded the observed disparity (152\%), meaning the algorithms partially \textit{compressed} rather than amplified inequality. Removing socioeconomic status from the model changed almost nothing, because cognitive scores already encode the same disadvantage. Exploratory intersectional analysis suggested the model may function as a poverty detector, though small subgroup sizes preclude definitive conclusions. Attempts to correct disparities reduced gaps but at substantial cost to accuracy, consistent with known impossibility results. These findings demonstrate that fairness failures in early childhood prediction are not fixable by choosing a better algorithm. They reflect the institutional conditions that produce the data, and addressing them requires upstream intervention in the educational structures that generate inequality before it reaches the model.
\end{abstract}
