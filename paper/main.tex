...
\begin{abstract}
We conducted a comprehensive fairness audit of seven machine learning algorithms predicting 5th-grade academic risk from early childhood data (ECLS-K:2011; complete-case $N = 9{,}104$; multiple imputation sensitivity on $N = 18{,}151$). All seven algorithms---spanning classical regularized models and state-of-the-art gradient boosting---converged on nearly identical performance (AUC range = 0.011), indicating that fairness failures are properties of the prediction task, not model architecture. Bootstrap confidence intervals confirmed significant disparities: Hispanic students were identified at 2.5 times the rate of White students, and Black students experienced 3.35 times higher calibration error. Additional longitudinal data improved accuracy (AUC: 0.799 to 0.831) without resolving these gaps, and fairness compliance was threshold-dependent (satisfied at only one of four tested cut-points). Counterfactual decomposition revealed that differences in baseline feature distributions account for more than 100\% of the Hispanic--White TPR gap (152\% averaged across reading and math)---meaning the raw distributional divergence would produce an even larger gap than observed, with the model partially compressing rather than amplifying it. SES-removal ablation confirmed that this structural encoding is mediated through cognitive scores ($\Delta$AUC = 0.003), and exploratory intersectional analysis produced a preliminary pattern consistent with the model functioning as a poverty detector, though key subgroup estimates rest on cell sizes too small for definitive conclusions. Both post-processing and in-processing mitigation reduced disparities but at substantial accuracy cost (AUC penalty = 0.21--0.23), consistent with known impossibility results. These findings demonstrate that fairness disparities in early childhood prediction are not resolvable through algorithmic selection alone, pointing toward institutional rather than merely technical intervention.
\end{abstract}
...
\begin{itemize}
    ...
    \item We provide empirical evidence that the ``poverty detector'' pattern is mediated through cognitive scores rather than SES directly (SES-removal ablation: $\Delta$AUC = 0.003), and that both post-processing and in-processing mitigation reduce disparities only at substantial accuracy cost.
...
False positive rate disparities were also substantial: Black students experienced an FPR of 0.095 [0.052, 0.133], compared to 0.011 [0.006, 0.017] for White students---8.5 times higher (see also Supplementary Figures S7--S8 for ROC and calibration curves by group).
...
The near-doubling of Black TPR (0.30 to 0.53) under MICE indicates that complete-case analysis systematically under-represents at-risk minority students. The two sets of estimates address different deployment contexts: complete-case results apply to stable-enrollment populations, while MICE estimates project performance on the full enrolled population including mobile students.
...
Seven models converged on nearly identical performance (AUC range = 0.011) and exhibited the same pattern of differential performance across racial groups---the convergence itself constituting evidence that fairness failures are properties of the data, not model architecture \citep{grinsztajn2022tree}. The dimensions of inequity were multiple and compounding: Hispanic students were identified at 2.5 times the rate of White students (confirmed by non-overlapping bootstrap CIs); Black students experienced 8.5 times higher FPR and 3.35 times higher calibration error \citep{pleiss2017calibration}; additional longitudinal data improved accuracy without resolving these gaps; and fairness compliance was threshold-dependent. Both mitigation strategies reduced disparities but at substantial accuracy cost (AUC penalty = 0.21--0.23), consistent with impossibility results \citep{chouldechova2017fair}. SES-removal ablation ($\Delta$AUC = 0.003) confirmed that the poverty-detector pattern is mediated through cognitive scores, and counterfactual decomposition showed that the structural component accounts for more than 100\% of the Hispanic--White TPR gap (152\% averaged across domains), with the model compressing rather than amplifying distributional divergence. MICE analysis on the full sample ($N = 18{,}151$) confirmed that differential performance persists after correcting for attrition.
