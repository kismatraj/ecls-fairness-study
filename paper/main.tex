...
\begin{itemize}
    ...
    \item We conduct a counterfactual decomposition of the Hispanic--White TPR gap, demonstrating that differences in baseline feature distributions are statistically associated with more than 100\% of the observed disparity---converting the claim of ``persistent disparities'' into an empirically decomposed explanation of mechanism.
\end{itemize}
...
Three features of the decomposition support its robustness. First, the structural component exceeds 100\% in both outcome domains independently, with non-overlapping bootstrap confidence intervals, constituting a cross-domain replication on different outcome measures within the same cohort. Second, the quantitative convergence is striking: 151.7\% and 153.6\% structural, $-$51.7\% and $-$53.6\% model-induced---nearly identical despite different outcome variables, feature weights, and prevalence rates. Third, the decomposition's central finding---that disparities originate in feature distributions rather than model mechanics---is independently corroborated by the seven-model convergence (if the model were the source, different architectures would produce different disparities) and the SES-removal ablation (if any single feature were the lever, removing it would matter).

Feature-level decomposition identified kindergarten cognitive scores as the dominant statistical contributor: the four baseline cognitive scores (fall and spring kindergarten reading and math) were together associated with 88.7\% of the reading TPR gap and 128.9\% of the math TPR gap. SES quintile contributed 28.6\% (reading) and 46.6\% (math). The cross-domain consistency is striking: structural components of 151.7\% and 153.6\%, model-induced components of $-$51.7\% and $-$53.6\%, and the same qualitative pattern across both outcome domains. This convergence provides strong evidence that the observed TPR disparities are a predictable consequence of the distributional properties of the input features---that is, of the educational inequality encoded in the data \textit{given} the outcome definition, feature set, and threshold that were chosen. The structural encoding is real, but it is mediated by institutional decisions about what to measure and how to define risk.
...
For policymakers, this means that algorithmic EWS should be understood as \textit{diagnostic instruments} that reveal the contours of institutional inequality, not as neutral tools that can be tuned to produce equitable outcomes. The convergence finding is itself the diagnosis: when every algorithm fails in the same way, the system---not the software---requires repair. This conclusion aligns with the sociotechnical perspective advanced by \citet{selbst2019fairness}: treating fairness as a property of the algorithm alone---rather than of the sociotechnical system encompassing data generation, model deployment, and institutional response---leads to the framing trap, in which technical interventions address symptoms while leaving structural causes intact. Our decomposition results provide direct empirical evidence for this concern: 152\% of the TPR gap is statistically attributable to the structural properties of the input data, meaning that the disparity is already present in the feature distributions before model training begins---though we note that the choice of features, outcome definition, and threshold are themselves institutional decisions that shape what gets encoded \citep{green2022flaws}. Similarly, \citet{eubanks2018automating}'s observation that automated systems in public services tend to intensify surveillance of disadvantaged populations finds a precise analog in our results: the model identifies at-risk Hispanic students at 2.5 times the rate of White students not because of algorithmic bias but because the data faithfully encodes the cumulative disadvantage that Hispanic children have experienced by kindergarten. In the language of \citet{bowker2000sorting}, the risk score imposes a classification that renders socioeconomic disadvantage legible to the institution---but in doing so, it reifies a particular reading of inequality as individual-level risk rather than structural condition. The governance question is therefore not whether the classification is accurate (it is) but whether the institutional response it triggers addresses or perpetuates the underlying stratification.

More broadly, our findings refine the sociotechnical systems framework by identifying a mechanism through which abstraction traps operate empirically. \citet{selbst2019fairness} theorized these traps at the level of problem formulation---how researchers define fairness, scope the system, and abstract away context. Our decomposition reveals that the same dynamic operates at the \textit{statistical} level: when institutional stratification produces group-level differences in feature distributions, the act of training a predictive model on those features necessarily encodes that stratification into model outputs, regardless of the algorithm chosen. This is what we mean by structural encoding---and it is distinct from several related concepts. It differs from \textit{algorithmic bias}, which implies the model introduces disparity; from \textit{proxy discrimination}, which implies a specific feature mediates the effect; from \textit{omitted variable bias}, which assumes that including additional covariates would explain away the disparity; and from \textit{selection bias}, which attributes differential outcomes to non-random sampling. Structural encoding is more fundamental: it holds even when the feature set is complete (no omitted variables), the sample is representative (no selection bias), and the model is optimal (no algorithmic bias). It operates through the joint distribution of all features simultaneously, as evidenced by the SES-removal ablation's null result and the decomposition's finding that cognitive scores alone are associated with 89--129\% of the gap. The theoretical contribution is thus to identify a specific, empirically measurable pathway by which \citet{selbst2019fairness}'s abstraction traps manifest in deployed systems: the statistical encoding of institutional stratification in the data-generating process, which no downstream algorithmic intervention can fully reverse.
